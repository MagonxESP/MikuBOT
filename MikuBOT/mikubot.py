...
class MikuBOT(Bot):
    ...
    def send_np(self, user, beatmap_link: str):
        ...
        if groups:
            ...
            main.logger.info("Matches found. Markdown generated: " + markdown)

            for channel in osu_user.channels:
                channel_id = int(channel.channel_id)
                discord_channel = self.get_channel(channel_id)
                main.logger.info("Sending /np to channel_id {}".format(channel_id))

                if discord_channel is not None:
                    asyncio.run_coroutine_threadsafe(discord_channel.send(markdown), self.loop)
                    main.logger.info("OK Sending /np to channel_id {}".format(channel_id))
                else:
                    main.logger.error("channel_id {} does not exists, aborting".format(channel_id))
